...
Hey everyone! Glad to see you here again.

Today, I'm thrilled to share an adventure I had discovering a critical flaw on the MyTrainPal website--an open redirect vulnerability that could potentially lead to account takeover. It's one of those bugs that seems small at first glance but can be a real headache if it falls into the wrong hands.

I was just having an ordinary day, sipping my coffee and poking around the MyTrainPal login page, when I stumbled upon something interesting. A vulnerability so subtle yet powerful, it could redirect unsuspecting users to malicious websites. The sneaky culprit? The `next` parameter in the login URL.
